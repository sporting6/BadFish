...
public class MoveParser {
    ...
    public Move getMove(String moveStr) {
        ...
        if (move.getDisplacement().getMoveable().getClass() != Pawn.class) {
            throw new InvalidMoveException(moveStr + " is not a pawn move", moveStr);
        }
        ...
        if (pieceClass == Pawn.class) {
            throw new InvalidMoveException(moveStr + " promotes to a pawn", moveStr);
        }
        if (pieceClass == King.class) {
            throw new InvalidMoveException(moveStr + " promotes to a King", moveStr);
        }
    ...
    private Move getMoveWithoutPromotion(String moveStr) {
        ...
        if (columnIndex < 0 || columnIndex >= maxColumns) {
            if (columnChar != CAPTURE_SEPARATOR) {
        ...
        if (!game.getPosition().getBoard().isValid(target)) {
            throw new IllegalArgumentException("Invalid move: " + moveStr);
        }
        ...
        if (possibleMoves.isEmpty()) {
            throw new InvalidMoveException(moveStr + " is not a legal move (" + columnIndex + "," + rowIndex + ", capture=" + filterCapture + ", " + pieceClass.getSimpleName() + ")", moveStr);
        }
        if (possibleMoves.size() == 1) {
            return possibleMoves.get(0);
        }
        if (possibleMoves.size() > 1 && ambigousMoveCharFilter == 0) {
            throw new InvalidMoveException(moveStr + " is ambiguous (" + possibleMoves.size() + " possible moves for " + colorToMove + ")", moveStr);
        }
        ...
        if (filteredMoves.isEmpty()) {
            throw new InvalidMoveException(moveStr + " does not exist (filtered out of " + possibleMoves.size() + " possible moves)", moveStr);
        }
        if (filteredMoves.size() > 1) {
            throw new InvalidMoveException(moveStr + " is [still] ambiguous (" + possibleMoves.size() + " possible moves for " + colorToMove + ")", moveStr);
        }
    ...
    public static String getMoveString(Move move) {
        ...
        if (moveable instanceof Pawn) {
            ...
            for (Moveable otherMoveable : position.getMoveables(moveable.getColor())) {
                if (otherMoveable == moveable) {
                    continue;
                }
                if (otherMoveable.getClass() != moveable.getClass()) {
                    continue;
                }
                if (!position.canBeReachedBy(newLocation, otherMoveable)) {
                    continue;
                }
            ...
            if (isAmbigous) {
                if (ambigousRow || !ambigousColumn) {
                    ambiguousStr += ((char) ('a' + oldLocation.getColumn()));
                }
                if (ambigousColumn) {
                    ambiguousStr += "" + (oldLocation.getRow() + 1);
                }
            }
        }
