...
public class Position {
    ...
    public static synchronized int getGcs() {
        return gcs;
    }
    ...
    public static synchronized void incGC(){
        gcs++;
    }
    public static synchronized void decGC(){
        gcs--;
    }
    ...
    private void computeLegalMoves() {
        ...
        for (Entry<Coordinate, Moveable> entry : moveables.entrySet()) {
            ...
            if (moveable.getColor() == colorOnMove && moveable instanceof King) {
                if (king != null) {
                    throw new IllegalStateException("Multiple " + colorOnMove + " king in position");
                }
                king = (King) moveable;
            }
        }
        ...
        for (Entry<Coordinate, Moveable> entry : moveables.entrySet()) {
            Moveable moveable = entry.getValue();
            if (moveable.getColor() != colorOnMove) {
                continue;
            }
            ...
            if (king != null) {
                for (Move move : allMoves) {
                    if (move.isPromotionNeeded()) {
                        move.setPromotion(new Queen(colorOnMove));
                    }
    ...
    public Position apply(Move move) {
        ...
        if (move.getLinkedDisplacements() != null) {
            move.getLinkedDisplacements().forEach(d -> p.increaseDisplacementCount(d.getMoveable(), 1));
        }
    ...
    public Moveable put(Coordinate location, Moveable moveable) {
        ...
        clearCache();
        if (moveable == null) {
            return moveables.remove(location);
        } else {
